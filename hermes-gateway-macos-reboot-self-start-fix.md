...
### 改动 1：`LimitLoadToSessionType` 改为版本条件输出

从 `generate_launchd_plist()` 的 plist 模板中提取该 key，改为 `_limit_load_section()` 函数动态生成：
- **macOS 26+**：省略该 key（避免自动加载静默失败 #23387）
- **macOS < 26**：保留 `Aqua + Background`（保持非 Aqua 登录场景的 Background-session 支持）
...
### 改动 2：`_launchctl_bootstrap()` 加入 macOS 26+ 路径

改造原有函数，不新建 `_launchd_load()`。macOS 26+ 路径用 `launchctl load plist` + `launchctl enable domain/label` 替代 `launchctl bootstrap`：

```python
def _launchctl_bootstrap(domain, plist_path, label, *, timeout=30):
    if _is_macos_26_or_later():
...
### 改动 3：统一所有 `launchctl bootstrap` 调用点（共 6 处）

| 位置 | 处理方式 |
|------|---------|
| `_launchctl_bootstrap()` 内部 | macOS 26+ 分支直接替换 ✅ |
| `launchd_install()` | 走 `_launchctl_bootstrap()` ✅ |
| `launchd_start()` — plist 丢失路径 | 走 `_launchctl_bootstrap()` ✅ |
| `launchd_start()` — kickstart 失败重载 | 走 `_launchctl_bootstrap()` ✅ |
| `launchd_restart()` — unloaded 分支 | macOS 26+: 走 `_launchctl_bootstrap()` / <26: bootout+bootstrap ✅ |
| `refresh_launchd_plist_if_needed()` — inline | 走 `_retry_→_launchctl_bootstrap()` ✅ |
| `refresh_launchd_plist_if_needed()` — 延迟重载脚本 | 版本条件 shell 字符串 ✅ |

### 改动 4：新增 8 个测试，覆盖 macOS 26+ 路径

包括底层 `_launchctl_bootstrap` 的 load/enable 单元测试、上层 `launchd_start`/`launchd_restart` 的集成测试、retry 循环的 macOS 26+ 行为验证。详见上方"PR Review 与迭代"章节。
